...
namespace BrickStoreBackend.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> RegisterEngineer([FromBody] UserDTO dto)
        {
            try
            {
                var newEngineer = await _userService.RegisterEngineerAsync(dto);
                return Created("", newEngineer);
            }
            catch (Exception)
            {
                return StatusCode(500, "Error registering engineer");
            }
        }
